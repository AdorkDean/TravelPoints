...
- (void)addAllSubView {
    ...
    for (NSInteger i = 0; i < self.numberOfColumn; i++) {
        ...
        //这里需要修改
        CGPoint indicatorLayerPosition = CGPointMake(titleLayerPosition.x + (textSize.width / 2) + 5, self.bounds.size.height * 0.5 + 2);
        
        CAShapeLayer *indicatorLayer = [self creatIndicatorLayer:indicatorLayerPosition color:self.itemTextUnSelectColor];
        indicatorLayer.borderColor = [UIColor clearColor].CGColor;
        [self.layer addSublayer:indicatorLayer];
        [self.currentIndicatorLayers addObject:indicatorLayer];
        
        // separatorLayer
        if (i != self.numberOfColumn - 1) {
            CGPoint separatorLayerPosition = CGPointMake(ceil((i + 1) * backgroundLayerWidth) +4, self.bounds.size.height * 0.5);
            
            CAShapeLayer *separatorLayer = [self creatSeparatorLayer:separatorLayerPosition color:[UIColor redColor]];
